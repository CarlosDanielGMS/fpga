...
int main(void)
{
    ...
    {
        print_str("Error: add fully connected failed");
        print_nl();
        while (1)
            ;
    }
    ...
    print_str("Allocating tensor...");
    print_nl();

    if (interpreter.AllocateTensors() != kTfLiteOk)
    {
        print_str("Error: allocate tensors failed");
        print_nl();
        while (1)
            ;
    }

    print_str("Success! Arena used: ");
    ...
    while (1)
    {
        float x = (count * 2.0f * 3.14159f) / 200.0f;
