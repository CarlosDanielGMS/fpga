...
int sensor_init(void) {
    i2c_start();
    if (!i2c_write_byte(SENSOR_I2C_ADDRESS << 1 | 0)) 
    { 
        i2c_stop(); return -1; 
    }
...
bool sensor_get_data(sensorData *d) {
    ...
    i2c_start();
    if (!i2c_write_byte(SENSOR_I2C_ADDRESS << 1 | 1)) { i2c_stop(); return false; } 
    ...
    d->humidity = (int16_t)(((uint64_t)raw_humidity * 10000) / 0x100000);
    d->temperature = (int16_t)((((uint64_t)raw_temperature * 20000) / 0x100000) - 5000);
